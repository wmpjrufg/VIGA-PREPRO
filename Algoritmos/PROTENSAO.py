...
def TENSAO_INICIAL(TIPO_PROT, TIPO_ACO, F_PK, F_YK, A_SCP):
    """
    ...
    P_IT0 = A_SCP * SIGMA_PIT0
    return SIGMA_PIT0, P_IT0
...
def VERIFICA_TENSAO_NORMAL(SIGMA, SIGMA_MAX):
    """
    Esta função verifica a restrição de tensão normal em peças estruturais
    
    Entrada:
    SIGMA       | Tensão normal calculada                        | kN/m² | float
    SIGMA_MAX   | Tensão máxima permitida                        | kN/m² | float

    Saída:
    G           | Valor da restrição                             |       | float
    """
    G = (SIGMA / SIGMA_MAX) - 1 
    return G  
...
def TENSAO_ACO(E_SCP, EPSILON, EPSILON_P, EPSILON_Y, F_P, F_Y):
    ...
    Entrada:
    E_SCP: Módulo de elasticidade do aço protendido (MPa)
    EPSILON: Deformação correspondente a tensão SIGMA desejada
    EPSILON_P: Deformação última do aço
    EPSILON_Y: Deformação de escoamento do aço
    F_Y: Tensão de escoamento do aço (MPa) 
    F_P: Tensão última do aço (MPa) 

    Saída:
    SIGMA: Tensão correspondente a deformação Deformação (kN/cm²)
    """
...
# DEFORMAÇÃO ARMADURA DE PROTENSÃO
def DEFORMACAO_ACO(E_SCP, SIGMA, EPSILON_P, EPSILON_Y, F_P, F_Y):
    ...
    Entrada:
    E_SCP: Módulo de elasticidade do aço protendido (MPa)
    SIGMA: Tensão correspondente a tensão EPSILON desejada (kN/cm²)
    EPSILON_P: Deformação última do aço
    EPSILON_Y: Deformação de escoamento do aço
    F_Y: Tensão de escoamento do aço (MPa) 
    F_P: Tensão última do aço (MPa) 
...
def AREA_ACO_FNS_RETANGULAR_SIMPLES(TIPO_CONCRETO, M_SD, F_CK, B_W, D, E_SCP, SIGMA, EPSILON_P, EPSILON_Y, F_P, F_Y):
    ...
    Entrada:
    M_SD: Momento de cálculo (kN.cm) 
    F_CK: Resistência característica à compressão (MPa)
    B_W: Lagura da seção (cm)
    D          | Altura útil da seção                             | m     | float
    E_SCP      | Módulo de Young do aço protendido                | kN/m² | float
    SIGMA_PI   | Tensão no aço após todas as perdas de protensão  | kN/m² | float
    EPSILON_P: | Deformação última do aço                         |       | float
    EPSILON_Y  | Deformação de escoamento do aço                  |       | float
    F_PK       | Tensão última do aço                             | kN/m² | float
    F_YK       | Tensão de escoamento do aço                      | kN/m² | float

    Saída:
    X: Linha neutra da seção medida da parte externa comprimida ao CG (cm)
    Z: Braço de alvanca (cm) 
    A_S: Área de aço necessária na seção (cm²) 
    """
...
# MOMENTO MINIMO PARA ARMADURA MÍNIMA LONGITUDINAL
def AREA_MINIMA_TRACAO_FNS(W_0, F_CK):
    """
    Esta função determina a área de aço mínima de tração

    Entrada:
    W_0: Módulo de resistência da seção transversal bruta de concreto, relativo à fibra mais tracionada (cm³)
    F_CK: Resistência característica à compressão (MPa)

    Saída:
    M_MIN: Momento mínimo para armadura mínima (kN.cm)
    """
...
def VERIFICACAO_CISALHAMENTO(F_CK, B_W, D, A_SW, F_YK, TIPO_VC, M_SDMAX, P_I, A, W, E_P):
    ...
    A_SW: Área de aço para cisalhamento (cm²/m)
